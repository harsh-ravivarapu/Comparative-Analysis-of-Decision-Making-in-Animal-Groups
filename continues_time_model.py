...
q = 0.5 # Weighting term for balancing preferred direction
and social interaction
speed = 1.0 # Speed of each individual
# Initialize positions and directions
positions = np.zeros((N, 2)) # Start from (0,0)
directions = np.array([[1, 1] for _ in range(N)]) # Initial
direction along 45-degree line
...
def update_directions(positions, directions, informed, a, r,
q, g1, g2, group1_indices, group2_indices):
...
if np.linalg.norm(positions[j] - positions[i]) !=
0 else np.zeros(2)
for j in range(N) if i != j and
np.linalg.norm(positions[j] - positions[i]) < a
], axis=0)
...
def plot_positions_with_trajectories(positions_history,
group1_indices, group2_indices, step, scale_factor=1.0):
plt.figure(figsize=(10, 10)) # Increase plot size
for i in range(N):
trajectory = np.array([pos[i] for pos in
positions_history[:step+1]]) * scale_factor
if i in group1_indices:
plt.plot(trajectory[:, 0], trajectory[:, 1],
c='red', alpha=0.6, linewidth=0.5) # Trajectory for subgroup
1
elif i in group2_indices:
plt.plot(trajectory[:, 0], trajectory[:, 1],
c='blue', alpha=0.6, linewidth=0.5) # Trajectory for
subgroup 2
else:
plt.plot(trajectory[:, 0], trajectory[:, 1],
c='green', alpha=0.6, linewidth=0.5) # Trajectory for rest
of the flock
plt.xlim(0, 100 * scale_factor)
plt.ylim(0, 100 * scale_factor)
plt.title(f'Flock Positions and Trajectories at Step
{step}',fontsize=15)
plt.xlabel('X Position',fontsize=15)
plt.ylabel('Y Position',fontsize=15)
plt.show()
# Use the updated plotting function in the simulation loop
scale_factor = 1.5 # Adjust as needed for better
visualization
for step in range(200):
if step % 20 == 0:
plot_positions_with_trajectories(positions_history,
group1_indices, group2_indices, step, scale_factor)
	
if np.linalg.norm(directions[j]) != 0 else
np.zeros(2)
for j in range(N) if i != j and
np.linalg.norm(positions[j] - positions[i]) < r
], axis=0)
...
new_directions[i] =
normalize(combined_direction.reshape(1, -1))[0]
elif i in group2_indices:
...
g1 = np.array([1, 0]) # Direction vector for the first
informed group
g2 = np.array([0, 1]) # Direction vector for the second
informed group
...
informed, a, r, q, g1, g2, group1_indices, group2_indices)
positions += directions * speeds[:, None]

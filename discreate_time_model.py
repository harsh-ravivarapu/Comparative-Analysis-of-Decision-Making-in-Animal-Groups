...
def update_theta(theta, ajl, N1, N2, K1, theta1, theta2):
    dtheta = np.zeros(N)
    for j in range(N):
        sum_term = sum(ajl[j, :] * np.sin(theta - theta[j])) / N
        if j < N1:
            dtheta[j] = np.sin(theta1 - theta[j]) + K1 * sum_term
        elif j < N1 + N2:
            dtheta[j] = np.sin(theta2 - theta[j]) + K1 * sum_term
        else:
...
def update_ajl(ajl, theta, K2, r):
    ...
    for j in range(N):
        for l in range(N):
            ...
            dajl[j, l] = K2 * (1 - ajl[j, l]) * ajl[j, l] * (rho_jl - r)
    return dajl
...
for t in range(steps):
    dtheta = update_theta(theta, ajl, N1, N2, K1, theta1, theta2)
...
plt.title('Directions of Individuals in the dirscrete-Time Model')
